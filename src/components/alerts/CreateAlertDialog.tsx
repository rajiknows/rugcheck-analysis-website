...
export default function CreateAlertDialog({
    ...
    const [comparison, setComparison] =
        useState<AlertComparison>("GREATER_THAN");
        ...
        mutate(payload, {
            onSuccess: (data) => {
                ...
                queryClient.invalidateQueries({
                    queryKey: ["alertsByUser", MOCK_USER_EMAIL],
                });
                queryClient.invalidateQueries({
                    queryKey: ["alertsByMint", mint],
                });
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>
                        Create Price Alert for {tokenName}
                    </DialogTitle>
                    <DialogDescription>
                        Current Price: ${currentPrice.toFixed(6)}. We'll notify
                        you when the condition is met.
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-3 items-center gap-4">
                        <Label htmlFor="comparison" className="col-span-1">
                            Condition
                        </Label>
                        <Select
                            value={comparison}
                            onValueChange={(value: string) =>
                                setComparison(value as AlertComparison)
                            }
                        >
                            <SelectTrigger
                                id="comparison"
